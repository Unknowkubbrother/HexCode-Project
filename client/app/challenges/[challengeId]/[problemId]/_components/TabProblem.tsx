...
export default function TabProblem({ problemData, setMyPoints }: { problemData: IProblem, setMyPoints: (points: number) => void }) {
  ...
  const [tabstate, setTabState] = useState<string>("testcase");
  ...
  const handleSubmission = async () => {
    ...
    if (sumPoints > (problemData.myMaxPoints ?? 0)) {
      setMyPoints(sumPoints);
    }
  ...
  return (
    <div className="w-full h-full relative">
     ...
     <div className={`w-full h-full flex justify-between gap-3 ${loading ? "blur select-none" : ""}`}>
        ...
        <div className="w-full h-fit flex flex-col gap-3">
          ...
          <Tabs value={tabstate} className="w-full h-fit max:h-[485px] bg-bgsecondary rounded-lg overflow-hidden">
            <TabsList className="w-full border-b-[1px] border-primary flex justify-start items-center py-1">
              <TabsTrigger value="testcase" onClick={() => setTabState("testcase")}>
              ...
              <TabsTrigger value="testresult" onClick={() => setTabState("testresult")}>
                <div className="flex items-center gap-2">
                  <span className="text-green-400">
                    <Terminal size={15} />
                  </span>
