    ...
    <main className="w-[60%] m-auto my-10">
      ...
      <div className="w-full flex gap-3 justify-start items-center">
        ...
        <div className="flex flex-col gap-2 mt-3">
          ...
          <Input id="title" type="text" placeholder="Title of the problem" className="w-[300px]" />
        </div>
      ...
      <div className="w-full mt-5 flex justify-between items-center pb-5 border-b-2">
        <div className="grid w-full max-w-sm items-center gap-1.5">
          ...
          <Input id="docs" type="file" accept=".pdf" className="mt-3" />
        </div>

        <div className="flex flex-col">
          <Label htmlFor="limit" className="text-sm ml-5"><span className="text-primary">LIMIT</span> LIST</Label>
          <nav>
            <ul className="flex gap-2 justify-start items-center">
              <li className="px-5 py-2 text-xs bg-background rounded-lg flex flex-col justify-center items-center gap-1">
                ...
                <Input id="time_limit" type="number" className="w-[100px] h-5 text-center" />
              </li>
              <li className="px-5 py-2 text-xs bg-background rounded-lg flex flex-col justify-center items-center gap-1">
                <div className="flex justify-center items-center gap-1">
                  <Database size={13} />
                ...
                <Input id="memory_limit" type="number" className="w-[100px] h-5 text-center" />
              </li>
              <li className="px-5 py-2 text-xs bg-background rounded-lg flex flex-col justify-center items-center gap-1">
                <div className="flex justify-center items-center gap-1">
                  <Database size={13} />
                ...
                <Input id="stack_limit" type="number" className="w-[100px] h-5 text-center" />
              </li>
              <li className="px-5 py-2 text-xs bg-background rounded-lg flex flex-col justify-center items-center gap-1">
                <div className="flex justify-center items-center gap-1">
                  <File size={13} />
                ...
                <Input id="filesize_limit" type="number" className="w-[100px] h-5 text-center" />
              </li>
            </ul>
          </nav>
        </div>
      ...
      <div className="w-full mt-5 flex flex-col justify-start items-start gap-3 pb-5 border-b-2">
        <div className="flex justify-start items-center gap-1.5">
          ...
          <Button variant="ghost" size='sm'
            onClick={() => setCountTestCases(countTestCases > 1 ? countTestCases - 1 : 1)}
          ><Minus /></Button>
          <span className="px-2 rounded-sm bg-bgsecondary">{countTestCases}</span>
          <Button variant="ghost" size='sm'
            onClick={() => setCountTestCases(countTestCases + 1)}
          ><Plus /></Button>
        </div>
        <div className="flex flex-col gap-3">
          {[...Array(countTestCases)].map((_, index) => (
            <div className="flex items-center space-x-2" key={index}>
            <Checkbox id={`testcase_${index+1}`} />
            <div className="w-full flex justify-between items-center gap-3">
              ...
              <div className="flex gap-3 justify-start items-center">
                ...
                <Input id={`output_testcase_${index+1}`} type="file" accept=".out" />
              </div>
            </div>
          </div>
          ))}
